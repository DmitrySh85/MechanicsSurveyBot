...
from static_text.static_text import CORRECT_ANSWER_TEXT
...
async def get_question_text(state: FSMContext, question_number: int):
    ...
    #answer_variants = [f"A) {data["questions"][question_number]["A"]}", f"B) {data["questions"][question_number]["B"]}", f"C) {data["questions"][question_number]["C"]}", f"D) {data["questions"][question_number]["D"]}"]
...
async def process_correct_answer(state: FSMContext):
    data = await state.get_data()
...
async def process_answer(callback_query: CallbackQuery, valid_answer, state: FSMContext):
    ...
    if int(answer) == int(valid_answer):
        ...
        await callback_query.message.answer(f"Вы не правы. Правильный ответ {'ABCD'[valid_answer]}.")
...
async def get_final_message_text(callback_query: CallbackQuery, valid_answers_count: int) -> None:
    ...
    message_text = f"Пользователь {user_first_name} {user_last_name} прошел опрос и дал {valid_answers_count} правильных ответ(ов) на 5 вопросов."
    return message_text
